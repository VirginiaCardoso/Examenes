...
function handler_formulario() {
	...
	$('#btn-modal-save, #btn-modal-reintentar').click(function(event) {
		...
		//DEBUG////////////////////////////////////////////////////////////////////////
		// $('#form-modificar').attr('action', $('body').data('site-url')+"/guias/guardar_modificaciones");
		// $('#form-modificar').submit();
		///////////////////////////////////////////////////////////////////////////////////
		
		$.ajax({ 
				data: $('#form-modificar').serialize(), // dato enviado en el post: codigo carrera
				type: "post",
				url: $('body').data('site-url')+"/guias/guardar_modificaciones", // controlador
				dataType: "json",
				timeout: 10000,
				...
				success: function(json) { 
					...
					if(response.ok) 
					{
						var msj = "Las modificaciones de las ponderaciones de la guia fueron guardadas correctamente. ";
						//DEBUG//////////////////////////////
					// msj += "<br/>"+JSON.stringify(response.data);
						...
						$('#btn-modal-ver').attr('href', $('#btn-modal-ver').data('link')+'/'+response.data.id_guia);
...
/*
 *	Se encarga de mostrar el validar el examen
...
function mostrar_modal(mensaje) {
	...
	{
		case 'warning-exit': 
						$('#modal-titulo').html("Abortar modificaciones");
		...
		case 'warning-save': 
						$('#modal-titulo').html("Guardar modificaciones");
		...
		case 'success':
						$('#modal-titulo').html("Modificaciones guardadas correctamente");
